...
const INITIAL_VISIBLE_COLUMNS = ["name", "title", "summary", "status", "lastUpdated", "actions"];
...
export default function SearchedTable() {
  ...
  const filteredItems = React.useMemo(() => {
    ...
    if (statusFilter !== "all" && Array.from(statusFilter).length !== statusOptions.length) {
      filteredContracts = filteredContracts.filter((contract) =>
        Array.from(statusFilter).includes(contract.status),
      );
    }
  ...
  }, [contracts, filterValue, statusFilter, typesFilter]);
      ...
      case "title":
        return (
          <div className="flex flex-col">
            ...
            <p className="text-bold text-tiny capitalize text-default-400">{contract.team}</p>
          </div>
        );
      case "status":
        return (
          <Chip className="capitalize" color={statusColorMap[contract.status]} size="sm" variant="flat">
            {cellValue}
          </Chip>
        );
      case "actions":
